...
variable in keys of the global dTemplate::parse hash. If we found it, then
we use its value for further processing.
...
If we did not find the referenced vlaue by the process described above,
...
variable and we are using the $dTemplate::parse{""} value. This is a
feature, not a bug, because $dTemplate::parse{""} is designed to be used
only to report errors or warnings, like this:
...
If you want to use the $dTemplate::parse{""} value as it was a normal
value, then set the $dTemplate::NOTASSIGNED_MODE variable to true. In this
case, the encoders and the formatter are called on the value of
...
date encoder, which expects the date to be an array ref of [ year, month,
day ], then you can do
this, e.g:
